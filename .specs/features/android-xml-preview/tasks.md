...
- ▶ **Verifier** — dispatched (session model, author≠verifier): spec-anchored outcome check + discrimination sensor → validation.md. Known-open gaps to confirm + rank: (1) Preprocessor comment-unawareness; (2) Degradation not wired into live path; (3) legacy Xamarin capital-cased resource dirs don't render (AD-001/Q6).
